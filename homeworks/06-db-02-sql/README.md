...
```
 При пересоздании нового контейнера с использование тех же volumes, данные из тома db: пробрасываются в контейнер в ```/var/lib/postgresql/data```, таким образом содержимое базы, а также все настройки при создании нового контейнера полностью сохраняются и востановления из бекапа в этом случае не требуется.
